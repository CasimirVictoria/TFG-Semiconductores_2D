...
Anem a suposar (simplificant encara més com són les interaccions entre àtoms) que un despla\c{c}ament longitudinal (radial, que estarà contés en el planol del cristall) o transversal en el plànol, sols genera una força radial o transversal també, es a dir, $\xi_n^{\alpha,\alpha'}=0$ tal y com es realitza en \cite{Balkanski_2000}. %Esta aproximació es coneix com la aproximació dels quatre veïns més pròxims \textit{4NNFC},% i es necessita considerar fins els cuarts veïns per donar compte dels resultats experimentals.
