...
Procedemos de manera análoga para los segundos y terceros vecinos.
Para los segundos vecinos (tenemos 6) del boro tenemos en cuenta que:
...
#+begin_src python :session :results none :exports none :async
  ...
  D_2__BB_xy=(Matrix([D_2__BB[i,j].factor() for i in range(2) \
  for j in range(2)]).reshape(2,2))
  D_2__BB_zz=D_2__BB[2,2]
  ...
  # for j in range(3)]).reshape(3,3)
  D_2__NN_xy=(Matrix([D_2__NN[i,j].factor() for i in range(2) \
  for j in range(2)]).reshape(2,2))
  D_2__NN_zz=D_2__NN[2,2]
  ...
  #for j in range(3)]).reshape(3,3)
  D_3__BN_xy=(Matrix([D_3__BN[i,j].factor() for i in range(2) \
  for j in range(2)]).reshape(2,2))
  D_3__BN_zz=D_3__BN[2,2]
  D_3__NB=1/sqrt(M_N*M_B)*(Phi_30__NB*fase(1,1)+Phi_3l__NB(2*pi/3)*fase(-1,-1) \
  +Phi_3l__NB(-2*pi/3)*fase(1,-1))
  D_3__NB_xy=(Matrix([D_3__NB[i,j].factor() for i in range(2) \
  for j in range(2)]).reshape(2,2))
  D_3__NB_zz=D_3__NB[2,2]
...
Debemos tener en cuenta en este punto que las  constantes de fuerza de fuerza asociadas a la interaccion de un átomo /??consigo mismo o con la totalidad del cristal??/ las excluimos con la ayuda de las condiciones impuestas por la invarianza respecto a la traslacion del cristal en su totalidad en las direcciones $x/z$ (cite:falkovsky08_symmet_const_phonon_disper_graph).


*** Matriz dinámica del cristal de BN (hasta terceros vecinos)
Puesto que las vibraciones fuera de plano estan desacopladas de las interplanares, podemos tratarlas independientemente, y en vez de representar la matriz dinámica mediante una matriz $6\times 6$ usamos 2 matrices:
Una de $2\times 2$ que da cuenta de las vibraciones en la dirección $\hat z$ y otra $4x4$ que nos informa de las frecuencias de los modos de vibracion en el plano del cristal.

\vspace{0.8cm}

#+begin_src python :session :results latex :exports results :async
  Dsup_xy=D_2__BB_xy.col_insert(2,D_1__BN_xy+D_3__BN_xy);
  Dinf_xy=(D_1__NB_xy+D_3__NB_xy).col_insert(2,D_2__NN_xy)
  D_xy=Dsup.row_insert(2,Dinf)
  D_zz=Matrix([[D_2__BB_zz,D_1__BN_zz+D_3__BN_zz],\
  [D_1__NB_zz+D_3__NB_zz,D_2__NN_zz]])
  ...
  def Matriu_com_a_Taula(Matriu,m,n):
    ...
    for i in range(m):
      for j in range(n):
	s += r"$ %d $ & $ %d $ & $ %s $" %(i+1, j+1, latex(Matriu[i,j]))
	s += r"\\ \hline"
  ...
  Matriu_com_a_Taula(D_zz,2,2)
#+end_src

#+RESULTS:
#+begin_export latex
\begin{tabular}{|c|c|c|}\hline$i$ & $j$ & $D_{i,j}$ \\ \hline$ 1 $ & $ 1 $ & $ \frac{\phi^{BB}_{2to} e^{i \left(- \frac{a q_{x}}{2} - \frac{\sqrt{3} a q_{y}}{2}\right)} + \phi^{BB}_{2to} e^{i \left(- \frac{a q_{x}}{2} + \frac{\sqrt{3} a q_{y}}{2}\right)} + \phi^{BB}_{2to} e^{i \left(\frac{a q_{x}}{2} - \frac{\sqrt{3} a q_{y}}{2}\right)} + \phi^{BB}_{2to} e^{i \left(\frac{a q_{x}}{2} + \frac{\sqrt{3} a q_{y}}{2}\right)} + \phi^{BB}_{2to} e^{i a q_{x}} + \phi^{BB}_{2to} e^{- i a q_{x}}}{M_{B}} $\\ \hline$ 1 $ & $ 2 $ & $ \frac{\phi^{BN}_{1to} e^{i \left(- \frac{a q_{x}}{2} + \frac{\sqrt{3} a q_{y}}{2}\right)} + \phi^{BN}_{1to} + \phi^{BN}_{1to} e^{- i a q_{x}}}{\sqrt{M_{B} M_{N}}} + \frac{\phi^{BN}_{3to} e^{i \left(- \frac{3 a q_{x}}{2} + \frac{\sqrt{3} a q_{y}}{2}\right)} + \phi^{BN}_{3to} e^{i \left(- \frac{a q_{x}}{2} - \frac{\sqrt{3} a q_{y}}{2}\right)} + \phi^{BN}_{3to} e^{i \left(\frac{a q_{x}}{2} + \frac{\sqrt{3} a q_{y}}{2}\right)}}{\sqrt{M_{B} M_{N}}} $\\ \hline$ 2 $ & $ 1 $ & $ \frac{\phi^{BN}_{1to} e^{i \left(\frac{a q_{x}}{2} - \frac{\sqrt{3} a q_{y}}{2}\right)} + \phi^{BN}_{1to} e^{i a q_{x}} + \phi^{BN}_{1to}}{\sqrt{M_{B} M_{N}}} + \frac{\phi^{BN}_{3to} e^{i \left(- \frac{a q_{x}}{2} - \frac{\sqrt{3} a q_{y}}{2}\right)} + \phi^{BN}_{3to} e^{i \left(\frac{a q_{x}}{2} + \frac{\sqrt{3} a q_{y}}{2}\right)} + \phi^{BN}_{3to} e^{i \left(\frac{3 a q_{x}}{2} - \frac{\sqrt{3} a q_{y}}{2}\right)}}{\sqrt{M_{B} M_{N}}} $\\ \hline$ 2 $ & $ 2 $ & $ \frac{\phi^{NN}_{2to} e^{i \left(- \frac{a q_{x}}{2} - \frac{\sqrt{3} a q_{y}}{2}\right)} + \phi^{NN}_{2to} e^{i \left(- \frac{a q_{x}}{2} + \frac{\sqrt{3} a q_{y}}{2}\right)} + \phi^{NN}_{2to} e^{i \left(\frac{a q_{x}}{2} - \frac{\sqrt{3} a q_{y}}{2}\right)} + \phi^{NN}_{2to} e^{i \left(\frac{a q_{x}}{2} + \frac{\sqrt{3} a q_{y}}{2}\right)} + \phi^{NN}_{2to} e^{i a q_{x}} + \phi^{NN}_{2to} e^{- i a q_{x}}}{M_{N}} $\\ \hline\end{tabular}
#+end_export


Notemos que podemos reescribir el elemento 1,1 como

#+begin_src python :session :results none :exports results
latex(D_zz[0,0].rewrite(cos).factor().simplify())
#+end_src

#+RESULTS:
#+begin_export latex
\begin{equation}
\frac{2 \phi^{BB}_{2to} \left(2 \cos{\left(\frac{a q_{x}}{2} \right)} \cos{\left(\frac{\sqrt{3} a q_{y}}{2} \right)} + \cos{\left(a q_{x} \right)}\right)}{M_{B}}
\end{equation}
#+end_export
...
Así, para las frecuencias de los fonones fuera de plano:
...
#+begin_src python :session :results latex :exports results :async
  ...
  latex(Matrix([D_Gamma_zz[i,j].factor() for i in range(2) for j in range(2)]).reshape(2,2))
#+end_src

#+RESULTS[ae17dc43b8c3e5d42d19c8ad34f963b3661440a2]:
#+begin_export latex
\begin{equation}
\label{eq:7}
\left[\begin{matrix}\frac{6 \phi^{BB}_{2to}}{M_{B}} & \frac{3 \left(\phi^{BN}_{1to} + \phi^{BN}_{3to}\right)}{\sqrt{M_{B} M_{N}}}\\\frac{3 \left(\phi^{BN}_{1to} + \phi^{BN}_{3to}\right)}{\sqrt{M_{B} M_{N}}} & \frac{6 \phi^{NN}_{2to}}{M_{N}}\end{matrix}\right]
\end{equation}
#+end_export
...
#+begin_src python :session :results latex :exports results :async
  #D_Gamma=D.subs([(q_x,0),(q_y,0),(M_B,B.mass*u),(M_N,N.mass*u)])
  P_Gamma_zz,D_Gamma_zz_diagonalitzada=D_Gamma_zz.diagonalize()
  #D_Gamma_zz.eigenvals(multiple=True)[0]
  #latex(D_Gamma_zz_diagonalitzada)
#+end_src
...
\begin{scriptsize}
\begin{align}
\omega_1^2&=\frac{3 \left(M_{B} \phi^{NN}_{2to} + M_{N} \phi^{BB}_{2to}\right)}{M_{B} M_{N}} - \frac{3 \sqrt{M_{B}^{2} \left(\phi^{NN}_{2to}\right)^{2} + M_{B} M_{N} \left(\phi^{BN}_{1to}\right)^{2} + 2 M_{B} M_{N} \phi^{BN}_{1to} \phi^{BN}_{3to} - 2 M_{B} M_{N} \phi^{BB}_{2to} \phi^{NN}_{2to} + M_{B} M_{N} \left(\phi^{BN}_{3to}\right)^{2} + M_{N}^{2} \left(\phi^{BB}_{2to}\right)^{2}}}{M_{B} M_{N}}\\
\omega_2^2&=\frac{3 \left(M_{B} \phi^{NN}_{2to} + M_{N} \phi^{BB}_{2to}\right)}{M_{B} M_{N}} + \frac{3 \sqrt{M_{B}^{2} \left(\phi^{NN}_{2to}\right)^{2} + M_{B} M_{N} \left(\phi^{BN}_{1to}\right)^{2} + 2 M_{B} M_{N} \phi^{BN}_{1to} \phi^{BN}_{3to} - 2 M_{B} M_{N} \phi^{BB}_{2to} \phi^{NN}_{2to} + M_{B} M_{N} \left(\phi^{BN}_{3to}\right)^{2} + M_{N}^{2} \left(\phi^{BB}_{2to}\right)^{2}}}{M_{B} M_{N}}
\end{align}
\end{scriptsize}
