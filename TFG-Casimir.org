...
#+begin_src python :session :results latex :exports both
  ...
  def R_hat(nuprima,nu,l_1,l_2):
    return (R_nu(nu,l_1,l_2)-R_nu(nuprima,0,0))/(R_nu(nu,l_1,l_2)-R_nu(nuprima,0,0)).norm()

  def propiedades_atomos(l_1, l_2):
    return [(k, m, i, j, R_hat(k,m,i,j), (R_nu(m,i,j)-R_nu(k,0,0)).norm())
...
#+begin_src python :session :results none :exports both
  alpha=Symbol('alpha')
  A1x, A1y, A2x, A2y = symbols("A1x, A1y, A2x, A2y")
...
\begin{equation}
-m_{1}\omega^2{A_1}_{z}={\alpha_z}_{p.v}
\end{equation}
