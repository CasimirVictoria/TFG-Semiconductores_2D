...
La idea que tengo, por ahora, es explicar el modelo de  Born y  von karman (1921), tal y como viene explicado en cite:brueesch82_phonon, pagina 4.
...
Vamos a suponer (por simplificar) que un desplazamiento longitudinal (radial, que estará contenido en el plano del cristal) o transversal (longitudinal, sea en el plano o perpendicular al plano) solo genera una fuerza radial o transversal.
...
#+begin_src python :session :results none :exports both
  ...
  Phi_1__BN=Matrix([[phi_1ti__BN,0,0],[0,phi_1r__BN,0],[0,0,phi_1to__BN]])
  ...
  def Phi_1l__BN(theta):
    return U(-theta)*Phi_1__BN*U(theta)
...
\newpage
\eject \pdfpagewidth=310mm \pdfpageheight=210mm

Por lo tanto, la contribución a matriz dinámica que tenemos por parte de la interacción de estos primeros vecinos es:

#+begin_src python :session :results none :exports both
  Phi_1__BN*fase(0,1)+Phi_1l__BN(2*pi/3)*fase(-1,0)+Phi_1l__BN(-2*pi/3)*fase(0,0)
  print_latex(Phi_1__BN*fase(0,1)+Phi_1l__BN(2*pi/3)*fase(-1,0)+Phi_1l__BN(-2*pi/3)*fase(0,0))
#+end_src
...
\newpage
\eject \pdfpagewidth=310mm \pdfpageheight=310mm
...
#+begin_src python :session :results none :exports both
  ...
  def Phi_1l__NB(theta):
    return U(-theta)*Phi_NB__1*U(theta)
 
  print_latex(Phi_1__NB*fase(0,-1)+Phi_1l__NB(2*pi/3)*fase(1,0)+Phi_1l__NB(-2*pi/3)*fase(0,0))
  #print_latex(Phi__NB1)
  #print_latex(Matrix([Phi__NB1[i,j].rewrite(cos).simplify() for i in range(3) for j in range(3)]).reshape(3,3))
#+end_src
...
Podemos observar que las vibraciones fuera de plano, en la dirección $z$ no estan acopladas a las interplanares, así como que $D^{NB}_{(1)}={D^{BN}_{(1)}}^*$
...
#+begin_src python :session :results none :exports both
  phi_r__2,phi_ti__2,phi_to__2=symbols('phi_r__2,phi_ti__2,phi_to__2')
  
  Phi_NN__2=Matrix([[phi_ti__2,0,0],[0,phi_r__2,0],[0,0,phi_to__2]])
  ...
  # Para el átomo que ocupa la posición l_1=-1,0
  def Phi_BB(theta):
    return U(-theta)*Phi_BB__2*U(theta)
 
  Phi__BB2=Phi_BB__2*fase(1,0)+Phi_BB(pi/3)*fase(1,1)+Phi_BB(-pi/3)*fase(0,-1) \
        +Phi_BB(pi)*fase(-1,0)+Phi_BB(2*pi/3)*fase(0,1)+Phi_BB(-2*pi/3)*fase(-1,-1)
  
  #print_latex(Matrix([Phi__BB2[i,j].rewrite(cos).simplify() for i in range(3) for j in range(3)]).reshape(3,3))
#+end_src
