...
* Pruebas varias (para probar el funcionamiento de org-mode, ...) :noexport:
...
Debemos notar que tal y como esta escrita la ecuación \ref{eq1}, sólo estamos considerando vibraciones dentro del plano del cristal. Como los átomo pueden moverse en la tres dimensiones debemos añadir una tercera componente a la matriz $\hat R_{\nu\prime\nu\vec l}\otimes\hat R_{\nu\prime\nu\vec l}$, y dado que podemos considerar que las vibraciones perpendiculares al plano están completamente desacopladas de las interplanares, simplemente añadimos un la unidad a la diagonal de la mariz, siendo nulos los otros elementos añadidos.
...
Fijemonos, por ahora, sólo en los primeros vecinos del átomo de Boro $\nu\prime=1$ 

#+begin_src python :session :results latex :exports none
    Atomos(1,1)[(Atomos(1,1)['Distancia']<0.9) & (Atomos(1,1)['Distancia']>0)&
		( Atomos(1,1)[r"$\nu\prime$"]==1)].to_latex(escape=False,index=False)
#+end_src
...
{A_1}_x}\\
{A_1}_y}\\
{A_1}_z}\end{pmatrix}\right)+\\
 &+\alpha_{1,2(1,0)}\left(\hat R_{1,2,(1,0)}\otimes\hat R_{1,2,(1,0)}\right)\cdot
\left(\begin{pmatrix}
{A_2}_x\\
{A_2}_y\\
{A_2}_z}
\end{pmatrix}\cdot e^{i (q_x R_{(1,0)}_x+q_y R_{(1,0)}_y)}-\begin{pmatrix}
A_{1_x}\\
A_{1_y}\\
A_{1_z}\end{pmatrix}\right)\right)\\
